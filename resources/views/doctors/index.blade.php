    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div class="bg-white rounded-2xl shadow p-4 text-center border-b-4 border-green-500">
            <p class="text-3xl font-extrabold text-green-600">
                {{ $doctors->getCollection()->filter(fn($d) => $d->is_available)->count() }}
            </p>
        ...
        <div class="bg-white rounded-2xl shadow p-4 text-center border-b-4 border-purple-500">
            <p class="text-3xl font-extrabold text-purple-600">
                {{ $doctors->getCollection()->pluck('specialization')->unique()->count() }}
            </p>
                ...
                <table class="w-full text-sm">
                    <thead class="bg-gray-50 border-b border-gray-100">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-semibold text-gray-600">Fee (TSh)</th>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex gap-2 flex-wrap">
                                    ...
                                    <form method="POST"
                                          ...
                                          onsubmit="return confirm('⚠️ Delete Dr. {{ addslashes($doctor->user->name) }}? This cannot be undone!')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                                class="bg-red-100 text-red-700 px-3 py-1.5 rounded-lg hover:bg-red-200 font-semibold text-xs transition">
                                            🗑️ Delete
                                        </button>
                                    </form>
                ...
                <div class="p-4">
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-3 text-xs">
                        <div class="bg-gray-50 rounded-lg p-2 text-center">
                            <p class="font-bold text-gray-700">{{ $doctor->qualification }}</p>
                            <p class="text-gray-400 mt-0.5">Qualification</p>
                        </div>
                        <div class="bg-gray-50 rounded-lg p-2 text-center">
                            <p class="font-bold text-gray-700">
                                {{ $doctor->experience ? $doctor->experience . ' yrs' : '—' }}
                            </p>
                            <p class="text-gray-400 mt-0.5">Experience</p>
                        </div>
                        <div class="bg-gray-50 rounded-lg p-2 text-center">
                            <p class="font-bold text-green-700">
                                TSh {{ number_format($doctor->consultation_fee) }}
                            </p>
                            <p class="text-gray-400 mt-0.5">Fee</p>
                        </div>
                    </div>
